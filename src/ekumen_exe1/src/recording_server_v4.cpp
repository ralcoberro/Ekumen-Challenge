...
double pFactor = 1.0f, pAngleFactor = 4.0f, distTolFactor = 0.1f, vel_min = 0.2f;
...
bool robotMoveToGoal(const geometry_msgs::Pose &goal, const double distTolerance)
{
   ...
   {
      if((pFactor * distance) > vel_min)
	vel_msg.linear.x = pFactor * distance;
      else
	vel_msg.linear.x = vel_min;
      ...
      if(angle > PI)
	angle = angle - (2*PI);
      vel_msg.angular.z = pAngleFactor * angle;

      ROS_INFO("Vel msg (Linear, angle): (%f, %f)", vel_msg.linear.x, angle);
